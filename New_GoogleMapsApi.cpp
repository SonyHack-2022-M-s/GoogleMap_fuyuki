...
String GoogleMapsApi::sendGetToGoogleMaps(String command) {
  ...
  if (client->connect(HOST, SSL_PORT)) {            
    //https communication to googlemap start  ->
    // Serial.println(".... connected to server");
    String a = "";
    char c;
    int ch_count = 0;
    client->println("GET " + command + "&key=" + _apiKey);  
    //GET command(all string) ->
    now = millis();
    avail = false;
    while (millis() - now < 1500) {
      while (client->available()) {  //client available ->
        char c = client->read();//client read to char c(maybe json to char)->
        // Serial.write(c);
